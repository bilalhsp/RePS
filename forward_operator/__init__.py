...
from .degredations import SuperResolution, Deblurring 
...
@register_operator(name='matrix_down_sampling')
class MatrixDownSampling(Operator):
    def __init__(self, resolution=256, scale_factor=4, device='cuda', sigma=0.05):
        super().__init__(sigma)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.resolution = resolution
        self.scale_factor = scale_factor
        self.H = SuperResolution(3, resolution, scale_factor, device)

        self.has_svd = True

    def __call__(self, x):
        B, C, H, W = x.shape
        return self.H.H(x).view(B, C, H // self.scale_factor, W // self.scale_factor)
...
@register_operator(name='cf_gaussian_blur')
class cfGaussianBlur(Operator):
    def __init__(self, kernel_size, intensity, device='cuda', sigma=0.05):
        super().__init__(sigma)
        self.device = device
        self.kernel_size = kernel_size
        ...
        k2d = scipy.ndimage.gaussian_filter(n, sigma=intensity)

        # Convert to equivalent 1D separable Gaussian kernel
        k1d = k2d[kernel_size // 2, :]
        k1d = k1d / k1d.sum()

        kernel = torch.from_numpy(k1d).float().to(device)
    ...
    def __call__(self, data):
        ...
        B, C, H, W = data.shape
        img_dim = H  # assuming square images
        x_vec = data.reshape(data.shape[0], -1)
        y_vec = self.H.H(x_vec)   # if H_functions defines H using U, S, Vt
        y = y_vec.reshape(data.shape[0], 3, img_dim, img_dim)
        return y
